...
def load_and_prepare_data(file_path):
    """Load AAPL data and calculate moving averages"""
...
def create_visualization(year_data, buy_signals, sell_signals):
    ...
    ax.plot(year_data['Date'], year_data['Adj Close'], 
            label='AAPL Adj Close', linewidth=2, color='blue', alpha=0.8)
    ...
    # Formatting
    ax.set_title('AAPL Trading Strategy - Buy/Sell Signals for 2015\n'
...
Profitable: {profitable_trades}/{total_trades}"""
    ...
    # Save the plot
    output_path = os.path.join('plots', 'AAPL_2015_signals.png')
...
def print_trade_details(buy_signals, sell_signals):
    """Print detailed trade information"""
    print("\n" + "="*80)
    print("DETAILED TRADE ANALYSIS FOR AAPL 2015")
    print("="*80)
...
def main():
    """Main function to generate AAPL 2015 visualization"""
    # Load AAPL data
    aapl_file = os.path.join('sd1', 'AAPL.csv')
    
    if not os.path.exists(aapl_file):
        print(f"Error: AAPL.csv not found at {aapl_file}")
        return
    
    print("Loading AAPL data...")
    df = load_and_prepare_data(aapl_file)
    ...
    fig = create_visualization(year_data, buy_signals, sell_signals)
    
    # Print detailed trade analysis
    print_trade_details(buy_signals, sell_signals)
