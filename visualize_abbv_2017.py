...
def find_buy_sell_signals(df, year=2017, buy_threshold=0.7, max_hold_days=40):
...
def create_visualization(year_data, buy_signals, sell_signals, ticker="ABBV", year=2017):
    ...
    # Formatting
    ax.set_title(f'{ticker} Trading Strategy - Buy/Sell Signals for {year}\n'
    ...
    if buy_signals and sell_signals:
        ...
        stats_text = f"""{year} Strategy Performance:
...
No completed trades
(Positions may be held into next year)"""
...
Price never reached buy threshold"""
        
        ax.text(0.02, 0.98, stats_text, transform=ax.transAxes, 
                verticalalignment='top', bbox=dict(boxstyle='round', 
                facecolor='lightcoral', alpha=0.8), fontsize=10)
    
    plt.tight_layout()
    
    # Save the plot
    output_path = os.path.join('plots', f'{ticker}_{year}_signals.png')
...
def print_trade_details(buy_signals, sell_signals, ticker="ABBV", year=2017):
    """Print detailed trade information"""
    print("\n" + "="*80)
    print(f"DETAILED TRADE ANALYSIS FOR {ticker} {year}")
    print("="*80)
    
    if not buy_signals:
        print(f"No buy signals found for {year}")
        print("The stock price never dropped 30% below the 100-day moving average.")
        return
    
    if not sell_signals:
        print(f"Buy signals found: {len(buy_signals)}")
        print("No completed trades (positions may be held into next year)")
        print("\nBUY SIGNALS:")
        print("-" * 40)
        for i, buy in enumerate(buy_signals, 1):
            print(f"Buy Signal {i}: {buy['date'].strftime('%Y-%m-%d')} at ${buy['price']:.2f}")
        return
...
def main():
    """Main function to generate ABBV 2017 visualization"""
    # Load ABBV data
    ...
    print("Finding buy/sell signals for 2017...")
    # Using strategy parameters from results: 30% below 100MA, max 40 days
    year_data, buy_signals, sell_signals = find_buy_sell_signals(df, 2017, 0.7, 40)
    ...
    fig = create_visualization(year_data, buy_signals, sell_signals, "ABBV", 2017)
    
    # Print detailed trade analysis
    print_trade_details(buy_signals, sell_signals, "ABBV", 2017)
